...
import { handleGenerate, handlePatchMissing, handleAsk, handleTrack, handleAnalytics, handleBriefing, handleLogs, handlePushSubscribe, handlePushUnsubscribe, handlePushSend, handlePushLog } from "@/lib/api/handlers";
...
const apiMiddleware = createMiddleware().server(async ({ next, request }) => {
  ...
  if (url.pathname === "/api/admin/push-log" && request.method === "GET") {
    return handlePushLog(request);
  }
  return next();
});
